...
import ru.javabegin.training.fastjava2.l6_listner_events.calc.CalcOperations;
...
public class CalcButtonActionListener implements ActionListener {
    ...
    public CalcButtonActionListener(MyJTextField jtxtChislo1, MyJTextField jtxtChislo2, MyJTextField jtxtResult) {
        //получаем тектовые поля и инициализируем их
    ...
    public void actionPerformed(ActionEvent e) {
        //проверяем что источник события JButton
        if (!(e.getSource() instanceof JButton)) {
            return;
        }

        //получаем кнопку по которой  сработало событие
        JButton btn = (JButton) e.getSource();

        //получаем числа из двух текстовых полей для ввода
        final double a = Double.parseDouble(jtxtChislo1.getText());
        final double b = Double.parseDouble(jtxtChislo2.getText());

        //в зависимости от того какая кнопка нажата вызываем соответствующую опер. и записываем в поле для результата
